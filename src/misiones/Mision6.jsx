// 🏆 MISIÓN 6 · JEFE FINAL: tu portfolio
// Armá un portfolio que junte todo.
//   1) <header> con un <h1> que muestre tu nombre desde una variable
//   2) Una lista <ul> con 3 o más skills
//   3) 2 o más proyectos con className="proyecto" → ¡creá un componente Proyecto y reutilizalo!
//   4) Un ternario que diga "Disponible para trabajar" o "No disponible"
//   5) Un <footer> con el año actual
//
// Recordá: un solo raíz (Fragment), llaves para JavaScript, className, componentes con Mayúscula.

function Mision6() {
  const nombre = "???";
  const disponible = true;

  return (
    <>
      <p>Empezá a construir tu portfolio acá 🚀</p>
    </>
  );
}

export default Mision6;
